...
const artifactsByRunUuid = (state = {}, action) => {
  switch (action.type) {
    case fulfilled(LIST_ARTIFACTS_API): {
      ...
      if (action.payload.root_uri.includes("ftp://")) {
        // Hacky way of fixing broken paths for ftp artifact servers
        // Take the ftp root uri
        let root_uri = action.payload.root_uri
        // Extract the filepath on ftp server
        let ftp_base_path = "/" + root_uri.split("/").slice(3, root_uri.length - 1).join("/")
        files.forEach((file) => {
          file.path = path.relative(ftp_base_path, file.path);
        });
      }
